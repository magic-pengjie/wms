package com.magic.card.wms.common.resolver;
...
/**
 * com.magic.card.wms.common.resolver
 * 请求JSON参数解析
 * @author : Mr.Zhang
 ...
 */
@Slf4j
public class RequestJsonParamResolver implements HandlerMethodArgumentResolver {
    ...
    public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception {
        ...
        RequestJsonParam parameterAnnotation = parameter.getParameterAnnotation(RequestJsonParam.class);
        //注解的value是JSON的key
        String key = parameterAnnotation.value();
        Object value;

        // 如果@RequestJsonParam注解没有设置value，则取参数名FrameworkServlet作为json解析的key
        if (StringUtils.isNotEmpty(key)) {
            value = jsonObject.get(key);
            // 如果设置了value但是解析不到，报错
            if (value == null && parameterAnnotation.required()) {
                throw RequestCheckException.instanceFormat(ResultEnum.req_body_json_param_requrie, key);
            }
        } else {
        ...
        if (isBasicDataTypes(parameterType) || String.class.isAssignableFrom(parameterType)) {
    ...
    private JSONObject getRequestBody (NativeWebRequest webRequest){
            ...
            if (jsonBody == null) {
                ...
                try {
                    ...
                    throw RequestCheckException.instance(ResultEnum.req_body_json_resolve);
                }

                if (StringUtils.isBlank(jsonBody)) {
                    throw RequestCheckException.instance(ResultEnum.req_body_json_null);
                }
